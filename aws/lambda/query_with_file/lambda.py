...
def validate_cognito_token(token):
    ...
    for pool_id in COGNITO_USER_POOL_IDS:
        ...
        try:
            ...
            print(f"[AUTH] Token validated successfully for pool {pool_id}, user: {user_email}")
            return user_email
...
def lambda_handler(event, context):
    ...
    """
    print("[QUERY] Received event:", json.dumps(event, default=str))
    ...
    try:
        ...
        if 'fileData' in body:
            ...
            print("[QUERY] Processing direct base64 upload")
            ...
            # Method 2: File already in S3
            s3_key = body.get('s3Key', '')
            print(f"[QUERY] Processing S3 file: {s3_key}")
        ...
        # Run bird detection
        print(f"[QUERY] Running detection on: {local_file_path}")
        file_type, detected_counts = detect_birds(local_file_path)
        print(f"[QUERY] Detection complete - Type: {file_type}, Counts: {detected_counts}")
        
        # Clean up S3 file if it exists
        if s3_key:
            try:
                s3.delete_object(Bucket=BUCKET_NAME, Key=s3_key)
                print(f"[QUERY] Deleted S3 file: {s3_key}")
            except Exception as e:
                print(f"[QUERY] Warning: Failed to delete S3 file: {e}")
        ...
        print(f"[QUERY] Detected species: {detected_species}")
        print(f"[QUERY] Querying database for matching files...")
...
def save_base64_to_temp(file_data_base64, file_ext):
    ...
    try:
        ...
        tmp_file.close()
        print(f"[QUERY] Saved base64 data to: {tmp_file.name}")
        return tmp_file.name
    except Exception as e:
        print(f"[QUERY] Error saving base64 file: {e}")
        raise
...
def download_from_s3(s3_key):
    ...
    try:
        ...
        tmp_file.close()
        print(f"[QUERY] Downloaded S3 file to: {tmp_file.name}")
        return tmp_file.name
    except Exception as e:
        print(f"[QUERY] Error downloading from S3: {e}")
        raise
...
def query_database_for_species(detected_species):
    ...
    try:
        ...
        print(f"[QUERY] Found {len(matching_items)} matching files")
        return matching_items
        
    except Exception as e:
        print(f"[QUERY] Error querying database: {e}")
        return []
...
def scan_all_media():
    ...
    try:
        ...
        print(f"[QUERY] Error scanning media table: {e}")
        return []
...
def format_results(items):
    ...
    for item in items:
        ...
        if url.startswith('s3://'):
            url = url.replace(f's3://{BUCKET_NAME}/', f'https://{BUCKET_NAME}.s3.amazonaws.com/')
