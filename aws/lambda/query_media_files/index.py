...
def get_item_by_thumb_https(thumb_https: str):
    s3_thumb = thumb_https.replace(f"https://{S3_BUCKET_NAME}.s3.amazonaws.com/", f"s3://{S3_BUCKET_NAME}/")
    items = scan_all(
...
def handle_query_by_thumbnail(payload):
    ...
    return success({"status": "success", "fullSizeUrl": s3_to_https(item["s3Url"])})
...
def handle_modify_tags(payload):
    ...
    if mode == "replace":
        ...
        item = (
            ...
            if "/thumb/" in url
            else table.get_item(Key={"mediaId": unquote_plus(url.split(f"https://{S3_BUCKET_NAME}.s3.amazonaws.com/")[-1])}).get("Item")
        )
        if not item:
            return error(404, "File not found")
        ...
        if not tag_updates:
            s3.delete_object(Bucket=S3_BUCKET_NAME, Key=media_id)
...
def handle_delete_files(payload):
    ...
    logs = []
    for url in urls:
        path = url.split(f"https://{S3_BUCKET_NAME}.s3.amazonaws.com/")[-1]
        if path.startswith("thumb/"):
            ...
            s3.delete_object(Bucket=S3_BUCKET_NAME, Key=item["mediaId"])
